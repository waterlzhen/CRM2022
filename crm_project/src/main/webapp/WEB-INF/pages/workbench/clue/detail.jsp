...
<script type="text/javascript">
	...
	/*	$(".remarkDiv").mouseover(function(){
		...
		//	收集参数
			var actIds=	$("#tBody input[type=checkbox]:checked");
			if (actIds.size()==0){
				alert("请选择需要删除市场活动")
				return ;
			}else {
				var ids ="";
				$.each(actIds,function (index,obj){//id=xxx&id=xxx&.....id=xxx&
					ids		+= "activityId="+this.value+"&";
				})
				ids+="clueId=${requestScope.clue.id}"
			//	发送请求
				$.ajax({
				    url:"workbench/clue/saveBindRelation.do",
				    data:ids,
				    type:"POST",
				    dataType:"json",
				    success:function (data){
				    		if (data.code=="1"){
				    			...
				    			$("#activityListTBody").append(htmlStr);
		...
		$("#activityListTBody").on("click","a",function (){
...
<body>
	...
	<div id="remarkListDiv" style="position: relative; top: 40px; left: 40px;">
		...
		<c:forEach items="${requestScope.remarkList}" var="clueRemark">
...
<%--		<div class="remarkDiv" style="height: 60px;">
	...
	<div>
		<div  style="position: relative; top: 60px; left: 40px;">
			...
			<div>
				<a href="javascript:void(0);" id="bindModalBtn" style="text-decoration: none;"><span class="glyphicon glyphicon-plus"></span>关联市场活动</a>
			</div>
		</div>
	</div>
