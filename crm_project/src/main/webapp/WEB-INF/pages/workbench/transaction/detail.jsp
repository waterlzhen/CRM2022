...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		阶段&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<c:forEach items="${applicationScope.stageList}" var="stageL">
<%--			遍历stageList ,依次显示每一个阶段对应的图标--%>
<%--				如果stageL 就处于当前交易阶段 显示map-marker 颜色为 绿色--%>
				<c:if test="${tran.stage eq  stageL.value}" >
					<span class="glyphicon glyphicon-map-marker mystage" data-toggle="popover" data-placement="bottom" data-content="${stageL.value}" style="color: #90F790;"></span>
					-----------
				</c:if>
			<%--如果stageL 处于当前交易阶段 “前面” 则 显示ok-circle 颜色为 绿色--%>
				<c:if test="${ tran.orderNo lt stageL.orderNo }">
					<span class="glyphicon glyphicon-record mystage" data-toggle="popover" data-placement="bottom" data-content="${stageL.value}"></span>
					-----------
				</c:if>
			<%--如果stageL 处于当前交易阶段 “后面” 则 显示ok-circle 颜色为 黑色--%>
			<c:if test="${ tran.orderNo gt stageL.orderNo}">
				<span class="glyphicon glyphicon-ok-circle mystage" data-toggle="popover" data-placement="bottom" data-content="${stageL.value}" style="color: #90F790;"></span>
				-----------
			</c:if>
